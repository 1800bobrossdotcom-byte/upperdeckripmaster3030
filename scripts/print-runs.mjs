// print-runs.mjs — per-card ERC-721 LENS edition size (the "denominations") for $UR3030.
//
// Each card in the field is a LENS: an ERC-721 render token (reads the Liquid
// Edition's market + burn state). A pack rips 7 lens copies; a card's print run is
// its 721 edition size. Rarer tier → smaller print run. Sized so the field's total
...
// per-tier print run (721 edition size per card of that rarity). Tunable knobs.
...
console.log('LENS PRINT RUNS · $UR3030 field (ERC-721 companion lens editions)\n');
...
console.log(`  · a card "sells out" when its 721 edition is fully minted (no more pulls); it "retires" when cumulative $UR3030`);
