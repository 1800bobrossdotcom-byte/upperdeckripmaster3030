...
// id), 33 minted hero 1/1s (11 auctioned + 11 gacha + 11 earned; docs/HERO-UNLOCKS.md names the
// eleven feats) + 67 render-only field cards + Lovebeing
// holder lens; packs burn the token (mint-once). ⛔ NO BURN PERCENTAGE IS PUBLISHED: the pack is a
// DOLLAR target, so the tokens it burns depend on the live price — see docs/PACK-PRICING.md,
// which is the headline risk and is stated wherever the burn is);
...
<style>
  ...
  *{ box-sizing:border-box; }
  ...
  /* ⚑ 16px CELLS IN A SIDEWAYS-SCROLLING TABLE BEATS 13px CELLS YOU CANNOT READ. Several of
     these cells are whole sentences ("maxTotalSupply() - totalSupply(), from the chain"), not
...
const whitepaper = `
  ...
  (<b>11 auctioned on SuperRare</b>, 11 pulled from packs, 11 earned in the games); the other <b>67 field cards</b> are <b>render-only lenses</b>
  ...
  <ul>
    ...
    <li><b>Rip &amp; collect</b> — a pack is a guided <b>buy of about $10 of $3030 — half burned, half to the studio</b>; you pull
  ...
  <b>measured, not modelled</b>: SuperRare ran the live mainnet previews at the full 3.3M supply on the
  ...
  <h2><span class="n">05</span>Packs · a $10 premium, escalating</h2>
  <p>A pack is the one <b>premium</b> action, and it uses only native curve operations: the site walks
  you through <b>buying about $10 worth of $3030</b> — <b>half burns, half funds the studio</b>, in one
  atomic call. It is <b>not</b> a token reprice —
  ...
  <div class="cols3">
    <div class="card house"><div class="co-h">33 hero lenses · minted now</div>
      <p style="margin:0"><b>11 auctioned</b> (bid on SuperRare) + <b>11 gacha</b> (pull a claim from a pack
      → mint) + <b>11 earned</b> (clear a named feat → signed voucher → mint). Real <b>1/1 ERC-721</b>
      tokens, wallet-signed. One owner each.</p></div>
  ...
  <ul>
    ...
    <!-- ⛔ THESE TWO NAME A GAME THAT IS OFF THE SHELF. NEON RONIN lost its slot to THE CITY on
         2026-08-03 (artist's call), so the whitepaper was telling collectors to win a 1/1 in a
         game the site does not link. The page promises them, so the ROUTE has to exist: the duel
         still runs at ronin.html and is linked here rather than quietly dropped.
         ⚠ Re-siting these two conditions in a live game is AUTHORSHIP, not a copy fix — it is the
         artist's call, and it is open. Until he makes it, a working link is the honest state;
         deleting two published titles would be the studio taking back a prize. -->
    <li><b>THREE CUTS</b> · <i><a href="ronin.html">Neon Ronin</a></i> — win a duel where <b>every landed hit was part of a combo</b>.</li>
    <li><b>NO SWORD</b> · <i><a href="ronin.html">Neon Ronin</a></i> — win a duel using <b>no slash at all</b>.</li>
    <li><b>HOUSE MONEY</b> · <i>The Arena</i> — take first on the podium <b>three times running</b>, never folding.</li>
    <li><b>THE LONG COUNT</b> · <i>any five games</i> — hold a <b>first-place finish in five different games</b>.</li>
  </ul>
  <p class="fine">⚠ <b>Neon Ronin is a retired game</b> — it left the arcade when THE CITY replaced it, and
  it is kept playable at <a href="ronin.html">ronin.html</a> precisely because two of these eleven titles
  live in it. Where those two conditions end up is the artist's call and is not settled.</p>
  ...
  deliberate: a balance can be borrowed for the length of one block, and a feat cannot.</p>
  <div class="callout amber"><b>How a title is actually awarded, stated plainly.</b> You record the run,
  you post it, the studio verifies it and signs a voucher, and <b>you</b> mint the card to <b>your</b>
  wallet and pay the gas — the studio never holds it for you. <b>And the studio is the judge.</b> Every
  score in these games lives in your own browser and can be edited in seconds, so a machine cannot
  referee this and we are not going to pretend one does. A card studio handing out its own titles is
  an honest arrangement; dressing it up as an oracle would not be.</div>
...
const tokenomics = `
  ...
  <table><tr><th>Tier</th><th>Pack allotment</th><th>price</th><th>$3030 per pack*</th><th>burned</th><th>to the studio</th></tr>
    <tr><td>Tier I</td><td>1,600 packs</td><td>$10</td><td>125</td><td>62.5</td><td>62.5</td></tr>
    <tr><td>Tier II</td><td>1,100 packs</td><td>$12</td><td>set at open</td><td>half</td><td>half</td></tr>
    <tr><td>Tier III</td><td>600 packs</td><td>$15</td><td>set at open</td><td>half</td><td>half</td></tr>
    <tr><td>Tier IV</td><td>260 packs</td><td>$20</td><td>set at open</td><td>half</td><td>half</td></tr></table>
  <p class="fine">*The pack is priced in <b>dollars</b>. The token amount is worked out from the live
  $3030 price on the day a tier opens and then <b>locked for that tier</b> — so it is a fixed, knowable
  number while you are buying, and only tier I's is knowable today. At the opening price of about
  <b>$0.08</b> a token, a $10 pack is 125 $3030. ≈3,560 packs across the four tiers.</p>
  ...
  <table><tr><th>Action</th><th>Cost</th><th>→</th></tr>
    <tr><td>rip a pack (field cards + rare gacha claim)</td><td>$10 → $20 by tier (§6)</td><td><span class="fire">🔥 half</span> · half studio</td></tr>
  ...
  in the direction people do not expect: <b>if the token gets more expensive, a $12 pack buys fewer
  tokens and burns less</b>. Any percentage printed here would be a promise about a price nobody
  controls, and it would go stale on its own. So the site reports <b>what has actually been burned</b>,
  read from the chain, instead of what we hope will be.</p>
  <p>The burn is real and permanent — it raises reserve-backing per surviving token — but it is
  <b>not</b> a scarcity engine and we do not present it as one. Reaching a 3× contraction would need
  packs of roughly <b>$99</b>. The studio chose playable pack prices over a deflation headline.</p>
...
const audit = `
  ...
  <ul>
    ...
    <li><b>Mint mechanism</b> — a claim/voucher redeemer for the 11 gacha lenses (pack burn) and the 11 earned
      lenses (signed game vouchers — kind 2 already covers all eleven, so no contract change).</li>
