...
window.S9Skin = (function () {
  ...
  /* ⚑ THE CC0 BODIES COME FIRST, and it is a quality call before it is a licence one. Measured
   * with `npm run stretch` at the same pose as everything else: lank 3.3x, lump 3.9x, squat 4.0x,
   * every one with ZERO triangles over 10x — the best-behaved bodies in the repo, next to oni's
   * 3.6x — and each is ~0.2 MB against oni's 0.9. A four-bot match now costs 0.8 MB of bodies
   * instead of 2.35, which is most of what made the roster short in the first place.
   * They are also the only ones with a clean provenance answer: every triangle is generated by
   * `scripts/blender/build-cc0-chars.py` from palettes measured off CC0-verified sources, so no
   * third-party mesh is committed anywhere. See models/cc0/README.md and docs/CC0-SOURCES.md.
   * ⚑ They bake correctly BECAUSE of .skn v2 — their proportions are not human (shoulders at
   * 0.76-0.78, not 0.80) and a canonical bind table would have torn them exactly like ronin. */
  const CAST = [
    { arch: 'cc0-lank',  bytes: 167288, mat: 1 },   // 1 cloth weave   — tall, wiry
    { arch: 'cc0-squat', bytes: 207272, mat: 7 },   // 7 wrap bands    — low, heavy
    { arch: 'cc0-lump',  bytes: 223736, mat: 5 },   // 5 skin          — soft mass
